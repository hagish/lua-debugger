...
local function has_breakpoint(file, line)
	if breakpoints_count == 0 then return nil end
...
local function debug_hook(event, line)
  ...
  elseif event == "return" then
    ...
    local file = debug_getinfo(2, "S").source
    
    local cached = merged_file_cache[file]
    if not cached then
			if string.find(file, "@") == 1 then
				cached = string.sub(file, 2)
			else
				cached = file
			end
			
			cached = merge_paths(lfs.currentdir(), cached)
			merged_file_cache[file] = cached
    end
...
local function debugger_loop(server)
  ...
  while true do
		--~ print("DEBUG wait for data")
    local line, status = server:receive()
    --~ print("DEBUG", line, status)
    ...
    elseif command == "FILE" then
      local _, _, file = string.find(line, "^[A-Z]+%s+(.+)$")
      if file then 
				local src = ""
				for line in io.lines(file) do
					if line then src = src .. line .. "\n" end
				end
				server:send("200 OK " .. string.len(src) .. "\n") 
        server:send(src)
      else
        server:send("400 Bad Request\n")
      end		  
...
function start()
  ...
  local server_socket = socket.connect(controller_host, controller_port)
  ...
  local server = {
		...
		receive = function(self)
						local data = server_socket:receive()
